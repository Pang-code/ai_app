...
from langchain_community.tools.gmail import search
...
history_aware_retriever = create_history_aware_retriever(
    deepseek_llm, retriever, contextualize_q_prompt
)
...
# 创建文档处理链
question_chain = create_stuff_documents_chain(deepseek_llm, qa_prompt)
...
resp1 = chain_with_message_history.invoke(
    {"input": "What is Task Decomposition?"},  # 用户输入问题
    config={
        "configurable": {"session_id": "abc123"}  # 使用会话ID "abc123" 保持对话
    }
)
...
# 调用会话式RAG链，询问"什么是任务分解？"
resp2 = chain_with_message_history.invoke(
